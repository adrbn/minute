...
export function BugReport({ onClose }: { onClose: () => void }) {
  ...
  const send = async () => {
    ...
    if (r.truncated) {
      ...
      toast(t('Journal complet copié : collez-le dans le ticket (Ctrl+V)'), 'info');
    }
  ...
  return (
    <div className="scrim" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="sheet report-sheet" role="dialog" aria-label={t('Signaler un problème')}>
        <div className="report-head">
          ...
          <div>
            ...
            <p>{t('Quelques mots suffisent : le journal technique aide à comprendre le reste.')}</p>
          </div>
        </div>
        <label className="report-field">
          <span>{t('En une phrase')}</span>
          <input
            className="field"
            autoFocus
            value={title}
            placeholder={t('Ex. : la transcription s’arrête après 10 minutes')}
        ...
        <label className="report-field">
          ...
          <textarea
            ...
            rows={4}
            value={what}
            placeholder={t('Ce que vous faisiez, ce que vous attendiez, ce qui s’est passé à la place.')}
            onChange={(e) => setWhat(e.target.value)}
          />
        </label>
        <div className="report-logs">
          <div>
            <b>{t('Joindre le journal technique')}</b>
            <span>{t('Version, système, réglages, erreurs récentes. Jamais de texte de réunion, de nom ni de clé.')}</span>
          </div>
          <Switch on={logs} onChange={setLogs} />
        </div>
        <button className={`disclosure ${open ? 'open' : ''}`} onClick={() => setOpen((o) => !o)}>
          <ChevronDown size={14} /> {t('Voir exactement ce qui sera envoyé')}
        </button>
        {open && <pre className="report-preview">{preview ?? '…'}</pre>}
        <p className="report-note">{t('Le ticket est public sur GitHub (un compte GitHub gratuit est nécessaire pour l’envoyer).')}</p>
        <div className="report-actions">
          <button className="btn" onClick={() => void copy()}>
            {copied ? <Check size={14} /> : <Copy size={14} />} {copied ? t('Copié') : t('Copier le rapport')}
          </button>
          <span className="grow" />
